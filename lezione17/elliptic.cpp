...
int main() {
  ...
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < n; j++) {
      solutions[i][j] = exp(-PI * x[i]) * sin(-PI * y[j]) +
                        S(x[i], y[j]) / 4 * (x[i] * x[i] + y[j] * y[j]);
  ...
  jacobi(solutions, xmin, xmax, ymin, ymax, n, n, S, 1e-7);
  ...
  gauss_seidel(solutions, xmin, xmax, ymin, ymax, n, n, S, 1e-7);

  ofstream file1;
  file1.open("gauss_seidel.dat");
  ...
  SOR(solutions, xmin, xmax, ymin, ymax, n, n, S, wsor, 1e-7);
